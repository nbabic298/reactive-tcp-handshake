package io.codifica.playground.reactive.tcp.handshake.client.support;
...
public enum ClientHandshakeState {
    ...
    public boolean receivedPayloadMatchesExpected(byte[] receivedPayload) {
        log.info("Comparing received payload (HEX): {} and expected response payload (HEX): {}",
                DataTypeUtil.bytesToHex(receivedPayload), DataTypeUtil.bytesToHex(responsePayload));
